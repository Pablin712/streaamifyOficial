...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        -- OBTENER EL COSTO QUE SE HA ASUMIDO EN EL MES DE UNA CUENTA ESPECIFICA DEL NEGOCIO
        ...
        BEGIN
            RETURN COALESCE(
                (SELECT SUM(MONTOCOS)
                FROM COSTOS
                WHERE IDCUE IN (SELECT IDCUE FROM CUENTAS WHERE IDcue = idcue_param)
        ...
        CREATE OR REPLACE FUNCTION contar_usuarios_perfil(idper_param VARCHAR)
        RETURNS INTEGER AS $$
        BEGIN
            RETURN COALESCE(
                (SELECT COUNT(*)
                FROM DETALLES_VENTA
                WHERE IDPER = idper_param
        ...
        BEGIN
            ...
            FROM VENTAS
            WHERE ventas.IDCLI = calcular_total_pagado_mes.idcli
        ...
        -- CALCULAR INGRESOS DE UN SERVICIO EN EL MES
        ...
        BEGIN
            IF IDSERVICIO = 'OTRO' THEN
                ...
                JOIN VALORES VA ON VA.IDVAL = P.IDVAL
                WHERE EXTRACT(MONTH FROM V.FECHAVEN) = MES
                AND VA.IDSER NOT IN (
            ...
            ELSE
                ...
                JOIN VALORES VA ON VA.IDVAL = P.IDVAL
                WHERE 
                    EXTRACT(MONTH FROM V.FECHAVEN)=MES AND
                    IDSERVICIO=VA.IDSER;
            END IF;
            RETURN TOTAL;
        END;
